...
func (m *Memberlist) probeNode(node *nodeState) {
	defer metrics.MeasureSinceWithLabels([]string{"memberlist", "probeNode"}, time.Now(), m.config.MetricsLabels)
	...
	if probeInterval > m.config.ProbeInterval {
		metrics.IncrCounterWithLabels([]string{"memberlist", "degraded", "probe"}, 1, m.config.MetricsLabels)
	}
...
func (m *Memberlist) gossip() {
	defer metrics.MeasureSinceWithLabels([]string{"memberlist", "gossip"}, time.Now(), m.config.MetricsLabels)
...
func (m *Memberlist) pushPullNode(a Address, join bool) error {
	defer metrics.MeasureSinceWithLabels([]string{"memberlist", "pushPullNode"}, time.Now(), m.config.MetricsLabels)
...
func (m *Memberlist) aliveNode(a *alive, notify chan struct{}, bootstrap bool) {
	...
	// Update metrics
	metrics.IncrCounterWithLabels([]string{"memberlist", "msg", "alive"}, 1, m.config.MetricsLabels)
...
func (m *Memberlist) suspectNode(s *suspect) {
	...
	// Update metrics
	metrics.IncrCounterWithLabels([]string{"memberlist", "msg", "suspect"}, 1, m.config.MetricsLabels)
	...
	fn := func(numConfirmations int) {
		...
		if timeout {
			if k > 0 && numConfirmations < k {
				metrics.IncrCounterWithLabels([]string{"memberlist", "degraded", "timeout"}, 1, m.config.MetricsLabels)
			}
...
func (m *Memberlist) deadNode(d *dead) {
	...
	// Update metrics
	metrics.IncrCounterWithLabels([]string{"memberlist", "msg", "dead"}, 1, m.config.MetricsLabels)
