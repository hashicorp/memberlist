...
func (m *Memberlist) probeNode(node *nodeState) {
	...
	if node.State == StateAlive {
		if err := m.encodeAndSendMsg(node.FullAddress(), pingMsg, &ping); err != nil {
			m.logger.Errorf("Failed to send ping: %s", err)
			if failedRemote(err) {
		...
		if buf, err := encode(pingMsg, &ping); err != nil {
			m.logger.Errorf("Failed to encode ping message: %s", err)
			return
		} else {
		...
		if buf, err := encode(suspectMsg, &s); err != nil {
			m.logger.Errorf("Failed to encode suspect message: %s", err)
			return
		} else {
			msgs = append(msgs, buf.Bytes())
		}
		...
		if err := m.rawSendMsgPacket(node.FullAddress(), &node.Node, compound.Bytes()); err != nil {
			m.logger.Errorf("Failed to send compound ping and suspect message to %s: %s", addr, err)
			if failedRemote(err) {
				goto HANDLE_REMOTE_FAILURE
			} else {
				return
	...
	case <-time.After(m.config.ProbeTimeout):
		...
		m.logger.Debugf("Failed ping: %s (timeout reached)", node.Name)
	}
...
HANDLE_REMOTE_FAILURE:
	...
	for _, peer := range kNodes {
		...
		if err := m.encodeAndSendMsg(peer.FullAddress(), indirectPingMsg, &ind); err != nil {
			m.logger.Errorf("Failed to send indirect ping: %s", err)
		}
	}
	...
	if (!disableTcpPings) && (node.PMax >= 3) {
		go func() {
			...
			if err != nil {
				m.logger.Errorf("Failed fallback ping: %s", err)
			} else {
	...
	for didContact := range fallbackCh {
		if didContact {
			m.logger.Warnf("Was able to connect to %s but other probes failed, network may be misconfigured", node.Name)
			return
		}
	}
	...
	m.logger.Infof("Suspect %s has failed, no acks received", node.Name)
	s := suspect{Incarnation: node.Incarnation, Node: node.Name, From: m.config.Name}
	m.suspectNode(&s)
}
...
func (m *Memberlist) Ping(node string, addr net.Addr) (time.Duration, error) {
	...
	m.logger.Debugf("Failed UDP ping: %v (timeout reached)", node)
	...
	for _, node := range kNodes {
		...
		if len(msgs) == 1 {
			...
			if err := m.rawSendMsgPacket(node.FullAddress(), &node, msgs[0]); err != nil {
				m.logger.Errorf("Failed to send gossip to %s: %s", addr, err)
			}
		} else {
...
func (m *Memberlist) pushPull() {
	...
	if err := m.pushPullNode(node.FullAddress(), false); err != nil {
		m.logger.Errorf("Push/Pull with %s failed: %s", node.Name, err)
	}
}
...
func (m *Memberlist) aliveNode(a *alive, notify chan struct{}, bootstrap bool) {
	...
	if len(a.Vsn) >= 3 {
		...
		if pMin == 0 || pMax == 0 || pMin > pMax {
			m.logger.Warnf("Ignoring an alive message for '%s' (%v:%d) because protocol version(s) are wrong: %d <= %d <= %d should be >0", a.Node, net.IP(a.Addr), a.Port, pMin, pCur, pMax)
			return
		}
	}
	...
	if m.config.Alive != nil {
		if len(a.Vsn) < 6 {
			m.logger.Warnf("ignoring alive message for '%s' (%v:%d) because Vsn is not present",
		...
		if err := m.config.Alive.NotifyAlive(node); err != nil {
			m.logger.Warnf("ignoring alive message for '%s': %s",
				a.Node, err)
			return
		}
	}
	...
	if !ok {
		...
		if errCon != nil {
			m.logger.Warnf("Rejected node %s (%v): %s", a.Node, net.IP(a.Addr), errCon)
			return
		}
		state = &nodeState{
			Node: Node{
		...
		if !bytes.Equal([]byte(state.Addr), a.Addr) || state.Port != a.Port {
			errCon := m.config.IPAllowed(a.Addr)
			if errCon != nil {
				m.logger.Warnf("Rejected IP update from %v to %v for node %s: %s", a.Node, state.Addr, net.IP(a.Addr), errCon)
				return
			}
			...
			if state.State == StateLeft || (state.State == StateDead && canReclaim) {
				m.logger.Infof("Updating address for left or failed node %s from %v:%d to %v:%d",
					state.Name, state.Addr, state.Port, net.IP(a.Addr), a.Port)
				updatesNode = true
			} else {
				m.logger.Errorf("Conflicting address for %s. Mine: %v:%d Theirs: %v:%d Old state: %v",
	...
	if !bootstrap && isLocalNode {
		...
		m.logger.Warnf("Refuting an alive message for '%s' (%v:%d) meta:(%v VS %v), vsn:(%v VS %v)", a.Node, net.IP(a.Addr), a.Port, a.Meta, state.Meta, a.Vsn, versions)
	} else {
...
func (m *Memberlist) suspectNode(s *suspect) {
	...
	if state.Name == m.config.Name {
		...
		m.logger.Warnf("Refuting a suspect message (from: %s)", s.From)
	...
	fn := func(numConfirmations int) {
		...
		if timeout {
			...
			m.logger.Infof("Marking %s as failed, suspect timeout reached (%d peer confirmations)",
...
func (m *Memberlist) deadNode(d *dead) {
	...
	if state.Name == m.config.Name {
		...
		if !m.hasLeft() {
			...
			m.logger.Warnf("Refuting a dead message (from: %s)", d.From)
